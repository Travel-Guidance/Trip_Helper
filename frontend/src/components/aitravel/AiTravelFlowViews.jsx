...
import {
  ...
  Users, DollarSign, Zap, Send, MessageCircle, Building2,
} from 'lucide-react'
import CalendarPicker from '../common/CalendarPicker'
import AiGenerationScheduleView from './AiGenerationScheduleView'
...
const CONTINENTS = [
  ...
  { key: 'oceania',    label: '오세아니아',   emoji: '🦘', countries: ['호주', '시드니', '멜버른', '골드코스트', '케언즈', '울루루', '뉴질랜드', '피지'] },
...
/* ──────────────────────────────────────────── 유틸 */
export function randomCode() { return Math.random().toString(36).slice(2, 7).toUpperCase() }
...
const STEP_META = [
  ...
  { label: '숙소',     icon: <Building2 className="w-4 h-4" /> },
]
const TOTAL_STEPS = 7

export function TravelFormWizard({ onSubmit, onBack }) {
  ...
  const addAccommodation = () => setAccommodations(prev => [
    ...prev,
    { id: Math.random().toString(36).slice(2), name: '', location: '', checkIn: '', checkOut: '' },
  ])
  ...
  const accsValid = hasAccommodation === false ||
    (hasAccommodation === true && accommodations.length > 0 &&
      accommodations.every(a => a.name.trim() && a.location.trim() && a.checkIn && a.checkOut))
  ...
  const canNext = [
    ...
    allStyles.length > 0,
    difficulty !== '',
    hasAccommodation !== null && accsValid,
  ]
  ...
  const goNext = () => {
    if (step < TOTAL_STEPS - 1) setStep(s => s + 1)
      ...
      difficulty,
      hasAccommodation,
      accommodations: hasAccommodation ? accommodations : [],
    })
  }
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-lg mx-auto px-5 py-8 pb-4">
          ...
          {step === 6 && (
            <div className="flex flex-col gap-6">
              <div>
                <h2 className="text-2xl font-black text-zinc-900 mb-1.5">숙소 예약 현황을 알려주세요</h2>
                <p className="text-zinc-500 text-sm">예약 정보가 있으면 숙소 위치 기반으로 동선을 최적화해드려요</p>
              </div>

              {hasAccommodation === null && (
                <div className="flex flex-col gap-3">
                  <button onClick={() => { setHasAccommodation(true); addAccommodation() }}
                    className="flex items-center gap-4 p-5 rounded-xl border-2 border-zinc-200 bg-white hover:border-zinc-900 transition-all text-left">
                    <div className="w-11 h-11 rounded-xl bg-zinc-100 flex items-center justify-center text-xl flex-shrink-0">🏨</div>
                    <div>
                      <p className="text-sm font-bold text-zinc-900">예, 숙소를 입력할게요</p>
                      <p className="text-xs text-zinc-400 mt-0.5">예약된 숙소 기준으로 동선을 짜드려요</p>
                    </div>
                  </button>
                  <button onClick={() => setHasAccommodation(false)}
                    className="flex items-center gap-4 p-5 rounded-xl border-2 border-zinc-200 bg-white hover:border-zinc-900 transition-all text-left">
                    <div className="w-11 h-11 rounded-xl bg-zinc-100 flex items-center justify-center text-xl flex-shrink-0">✨</div>
                    <div>
                      <p className="text-sm font-bold text-zinc-900">아니요, AI에게 맡길게요</p>
                      <p className="text-xs text-zinc-400 mt-0.5">AI가 최적 위치 숙소를 함께 추천해드려요</p>
                    </div>
                  </button>
                </div>
              )}
              ...
              {hasAccommodation === true && (
                <div className="flex flex-col gap-4">
                  <button onClick={() => { setHasAccommodation(null); setAccommodations([]) }}
                    className="text-xs text-zinc-400 underline self-start hover:text-zinc-600 transition-colors">
                    ← 다시 선택
                  </button>
                  {accommodations.map((acc, idx) => (
                    <div key={acc.id} className="border border-zinc-200 rounded-xl p-4 flex flex-col gap-3">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-zinc-500 uppercase tracking-wider">숙소 {idx + 1}</span>
                      ...
                      <input type="text" placeholder="숙소 이름 (예: 힐튼 시드니)"
                        value={acc.name} onChange={e => updateAccommodation(acc.id, 'name', e.target.value)}
                        className="w-full h-10 px-3 text-sm bg-zinc-50 border border-zinc-200 rounded-lg focus:outline-none focus:border-zinc-900 transition-all placeholder:text-zinc-400" />
                      <input type="text" placeholder="지역 / 위치 (예: 시드니 서큘러 키)"
                        value={acc.location} onChange={e => updateAccommodation(acc.id, 'location', e.target.value)}
                        className="w-full h-10 px-3 text-sm bg-zinc-50 border border-zinc-200 rounded-lg focus:outline-none focus:border-zinc-900 transition-all placeholder:text-zinc-400" />
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <label className="text-[10px] font-semibold text-zinc-400 uppercase tracking-wider mb-1 block">체크인</label>
                          <input type="date" value={acc.checkIn} min={startDate} max={endDate}
                            onChange={e => updateAccommodation(acc.id, 'checkIn', e.target.value)}
                            className="w-full h-10 px-3 text-sm bg-zinc-50 border border-zinc-200 rounded-lg focus:outline-none focus:border-zinc-900 transition-all" />
                        </div>
                        <div>
                          <label className="text-[10px] font-semibold text-zinc-400 uppercase tracking-wider mb-1 block">체크아웃</label>
                          <input type="date" value={acc.checkOut} min={acc.checkIn || startDate} max={endDate}
                  ...
                  <button onClick={addAccommodation}
                    className="flex items-center justify-center gap-2 w-full py-3 rounded-xl border-2 border-dashed border-zinc-300 text-zinc-400 hover:border-zinc-500 hover:text-zinc-600 transition-all text-sm font-medium">
                    <Plus className="w-4 h-4" /> 숙소 추가
                  </button>
                </div>
              )}
              ...
              {hasAccommodation === false && (
                <div className="flex flex-col gap-4">
                  <button onClick={() => setHasAccommodation(null)}
                    className="text-xs text-zinc-400 underline self-start hover:text-zinc-600 transition-colors">
                    ← 다시 선택
                  </button>
                  <div className="bg-zinc-50 rounded-xl p-6 border border-zinc-200 text-center">
                    <p className="text-3xl mb-3">✨</p>
                    <p className="text-sm font-bold text-zinc-900 mb-1">AI가 숙소를 함께 추천해드려요</p>
                    <p className="text-xs text-zinc-500">일정 생성 후 추천 숙소를 바로 예약할 수 있어요</p>
                  </div>
                </div>
              )}
            </div>
...
/* ════════════════════════════════════════════
   GROUP INVITE
════════════════════════════════════════════ */
export function GroupInvite({ roomCode, friends, tripInfo, onProceed }) {
...
/* ════════════════════════════════════════════
   FRIEND JOIN FORM
════════════════════════════════════════════ */
export function FriendJoinForm({ roomCode }) {
...
export function Loading({ info, onDone, onBack }) {
  const [step, setStep] = useState(0)
...
/* ════════════════════════════════════════════
   RESULT
════════════════════════════════════════════ */
export function Result({ info, planData, onReset }) {
