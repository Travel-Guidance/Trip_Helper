...
## 권역별 숙소 분리 원칙 (핵심)
관광 item은 같은 도시·같은 권역 안에 있어야 합니다. 단, 도시 이동일에는 이동 항목과 도착 도시 숙소 체크인만 예외로 허용합니다.
...
function buildCollabRagQuery({ dest, styles }) {
  const destination = String(dest || '').trim();
  const styleTerms = Array.isArray(styles) ? styles.join(' ') : '';
  const countryOnly = ['호주', '오스트레일리아', 'australia', 'au'].includes(destination.toLowerCase());
  return `${countryOnly ? '호주 여행' : `${destination} 호주 여행`} ${styleTerms}`.trim();
}
...
2. mustVisit은 일정의 일부 앵커로만 배치하고, 남은 시간은 여행 스타일·예산·동선에 맞는 다양한 장소로 채우세요.
3. 도시별 숙박 순서와 checkIn/checkOut을 먼저 결정하세요.
4. 같은 권역의 장소끼리 같은 날에 묶고, 도시 이동이 필요하면 이동일을 별도로 설정하세요.
5. 도시 이동일은 전날 숙소 출발 → 장거리 이동 → 도착 도시 숙소 체크인/도착으로 끝내세요.
6. 다음날은 반드시 전날 도착한 숙소에서 출발하세요.
7. 한 일차 안에 서로 다른 도시의 관광 장소를 절대 섞지 마세요.
