...
2. 이동일(transfer day)은 오직 아래 5개 이하 items만 허용합니다.
   ...
   - [기술적 이유] 항공편 item은 출발지 좌표, 도착 공항 item은 도착지 좌표를 가집니다. 관광 item이 하나라도 추가되면 이동일 판정이 해제되고, 1400~2200km에 달하는 좌표 격차가 동선 위반으로 자동 검출됩니다. 이것은 예외 없이 오류로 기록됩니다.
   - [잘못된 예] 멜버른 호텔 출발 / 멜버른 공항 / 멜버른→울루루 항공편 / 에어즈록 공항 도착 / 울루루 선셋 뷰잉 ← 6번째 item 추가 금지
   - [올바른 예] 멜버른 호텔 출발 / 멜버른 공항 / 멜버른→울루루 항공편 / 에어즈록 공항 도착 / 사막 팔라스 리조트 체크인 (총 5개, 관광 0개)
...
## JSON 출력 전 자가 검증 (필수 — 위반 시 재생성)
JSON을 출력하기 전 다음 5가지를 순서대로 직접 확인하세요. 하나라도 실패하면 수정 후 출력하세요.
1. 모든 이동일의 items 개수가 5개 이하인가?
2. 모든 이동일에 관광·식사·체험 항목이 단 하나도 없는가?
3. 장거리 이동(400km 이상) 당일 도착지 관광 항목이 없고, 관광은 다음날부터 시작하는가?
4. 각 일반 관광일의 모든 items lat/lng가 같은 도시·권역 안에 있는가?
5. 도시 이동 후 출발 숙소로 돌아오는 일정(A→B→A 패턴)이 없는가?
...
이유: 항공편 item(출발지 좌표)과 도착 공항 item(도착지 좌표) 사이의 거리가 1400~2200km입니다. 관광 item을 추가해 총 items가 6개 이상이 되면 이동일 판정이 해제되고 이 좌표 격차 전체가 오류로 검출됩니다.
잘못된 예: ...멜버른 공항 / 멜버른→울루루 항공 / 에어즈록 공항 도착 / 울루루 관광 ← 오류 발생
올바른 예: ...멜버른 공항 / 멜버른→울루루 항공 / 에어즈록 공항 도착 / 리조트 체크인 ← 정상
