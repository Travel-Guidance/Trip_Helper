...
## 무리한 요청 처리 원칙
- 사용자가 요청한 장소, 기간, 예산이 동시에 만족되지 않으면 절대 터무니없는 동선을 만들지 마세요.
- 일부 장소만 무리하면 가장 효율적인 핵심 루트로 일정을 만들고, 제외한 장소는 omittedPlaces에 "왜 제외했는지"와 "대안"을 적으세요.
- 전체 요청이 명백히 불가능하면 feasibility.status를 "impossible"로 두고, feasibility.message에 "해당 예산과 기간으로 입력하신 일정은 무리입니다."로 시작하는 안내를 넣으세요.
- impossible인 경우에도 무리한 전체 루트를 만들지 말고, days에는 사용자가 다시 선택할 수 있는 현실적인 축소 대안 일정 1개를 작성하세요.
- 예산은 항공권 제외 예산으로 해석하되, 호주 국내선/장거리 이동/투어가 많아 예산 초과가 예상되면 warnings에 명확히 표시하세요.
...
STEP 0 — Trip pacing before region selection:
  ...
  - Keep accommodations aligned with selected regions. For trips up to 14 days, use at most 3 accommodation bases.

STEP 0.5 — Feasibility decision:
  - Before writing days, decide feasibility.status:
    * "feasible": requested places mostly fit the period, budget, and region limit.
    * "needs_adjustment": a good itinerary is possible only after excluding one or more requested places.
    * "impossible": the requested mustVisit route is broadly incompatible with the period or budget.
  - If status is "needs_adjustment", write the best realistic itinerary and list every excluded request in omittedPlaces.
  - If status is "impossible", feasibility.message must start with "해당 예산과 기간으로 입력하신 일정은 무리입니다." Then suggest 2-3 concrete fixes in feasibility.suggestedAdjustments.
  - Never hide an omitted requested place. If a requested place is not in days[].items, it must appear in omittedPlaces.
...
Return JSON may include:
  "feasibility": { "status": "feasible|needs_adjustment|impossible", "message": "Korean user-facing explanation", "suggestedAdjustments": ["extend trip", "increase budget", "remove region"] },
  "includedPlaces": ["place or region name"],
  "omittedPlaces": [{ "name": "place", "reason": "why it does not fit this trip length/route", "alternative": "nearby or lower-cost replacement" }]
`;
}
...
먼저 기간/예산/권역 수로 feasibility를 판단한 뒤, 가능한 경우에만 가장 효율적인 루트로 days를 작성하세요.
...
꼭 방문할 곳 중 기간/예산/동선상 무리한 장소는 억지로 넣지 말고 omittedPlaces에 제외 이유와 대안을 넣으세요.
무리한 요청이면 warnings에도 "일정 조정 필요" 안내를 추가하세요.
...
note는 "왜 이 장소를 이 시간대에 배치했는지"가 드러나게 1~2문장으로 간결하게 작성하세요.
모든 식사/카페 항목은 실제 상호명을 사용하세요. isMeal:true 항목에 상호명이 없으면 그 항목은 무효입니다.
각 식사 항목의 note에는 왜 이 식당인지와 반드시 시도해볼 메뉴명을 짧게 포함하세요.
