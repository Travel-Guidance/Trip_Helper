...
router.post("/auth/signup", async (req, res, next) => {
  try {
    ...
    if (!email || !password || !name) {
      return res
        .status(400)
        .json({ error: "이메일, 비밀번호, 닉네임을 모두 입력하세요." });
    }
...
function getKakaoRedirectUri() {
  return (
    process.env.KAKAO_REDIRECT_URI || `${getFrontendUrl()}/auth/kakao/callback`
  );
}
  ...
  try {
    ...
    if (!clientId) {
      return res.status(500).json({
        error: "Kakao login is not configured.",
        detail:
          "Set KAKAO_CLIENT_ID in backend/.env to your Kakao REST API key.",
      });
    }
    ...
    if (!tokenRes.ok) {
      ...
      return res.status(502).json({
        error:
          "Failed to exchange Kakao authorization code for an access token.",
        detail: errBody,
    ...
    if (!profileRes.ok) {
      return res
        .status(502)
        .json({ error: "카카오 프로필을 가져오지 못했습니다." });
    }
...
router.get("/auth/google/profile", async (req, res, next) => {
  try {
    ...
    if (!code) {
      return res
        .status(400)
        .json({ error: "Google authorization code is missing." });
    }
    ...
    if (!tokenRes.ok) {
      ...
      return res.status(502).json({
        error:
          "Failed to exchange Google authorization code for an access token.",
        detail: errBody,
